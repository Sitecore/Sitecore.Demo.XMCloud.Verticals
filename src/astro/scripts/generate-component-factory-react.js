...
/*
  ...
  This mapping is used to construct the React hierarchy for the layout.
...
function generateComponentFactory() {
  ...
  // * under /src/components/integrations/react
  // * with a .tsx extension to define a component file
...
/**
 * Recursively iterates the given folderPath, creating a flat array of found .tsx file paths.
 * For example, given the following folder structure and using `/components` as the root folderPath:
 * /components/component0.tsx
 * /components/subfolder/component1.tsx
 *
 * The output would be:
 * ['component0.tsx', 'subfolder/component1.tsx']
 */
function extractReactFiles(folderPath) {
  let results = [];
  fs.readdirSync(folderPath).forEach((pathName) => {
    ...
    if (stat && stat.isDirectory()) {
      results = results.concat(extractReactFiles(computedPath));
    } else if (path.extname(computedPath).toLowerCase() === '.tsx') {
